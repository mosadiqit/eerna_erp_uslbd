...
class StockMove(models.Model):
    ...
    route_ids = fields.Many2many(
        'stock.location.route', 'stock_location_route_move', 'move_id', 'route_id', 'Destination route',
        ...
        for move in self:
            if not move.product_id:
                ...
                move.show_details_visible = (
                        ((consignment_enabled and move.picking_id.picking_type_id.code != 'incoming') or
        ...
        for move in other_moves:
            ...
            if move.reserved_availability:
                if move.reserved_availability != total_available:
                    ...
                    reserved_available = move.product_id.uom_id._compute_quantity(move.reserved_availability,
        ...
        if moves_error:
            ...
            for move in moves_error:
                user_warning += _('\n\n%s --> Product UoM is %s (%s) - Move UoM is %s (%s)') % (
                    move.product_id.display_name, move.product_id.uom_id.name, move.product_id.uom_id.category_id.name,
                    move.product_uom.name, move.product_uom.category_id.name)
        ...
        if propagated_date_field:
            ...
            for move in self:
                ...
                for move_dest in move_dest_ids:
                    ...
                    # in the past.
                    new_move_date = max(move_dest.date_expected + relativedelta.relativedelta(days=delta_days or 0),
                                        fields.Datetime.now())
    ...
    def _delay_alert_log_activity(self, mode, move_orig):
        ...
        if mode == 'auto':
            ...
            msg = _(
                "The scheduled date should be updated due to a delay on <a href='#' data-oe-model='%s' data-oe-id='%s'>%s</a>.") % (
                      doc_orig[0]._name, doc_orig[0].id, doc_orig[0].name)
    ...
    def _push_apply(self):
        for move in self:
            ...
            if not self.env.context.get('force_company',
                                        False) and move.location_dest_id.company_id == self.env.user.company_id:
    ...
    def _merge_moves_fields(self):
        ...
        return {
            ...
            'date_expected': min(self.mapped('date_expected')) if self.mapped(
                'picking_id').move_type == 'direct' else max(self.mapped('date_expected')),
    ...
    def check_if_move_come_from_transfer(self, picking_id=None):
        if not picking_id:
            return False
        picking_row = self.env['stock.picking'].sudo().search([('id', '=', picking_id)])
        if picking_row and 'INT' in (picking_row.name).split("/"):
            return True
        return False

    def check_if_move_come_from_deliver(self, picking_id=None):
        if not picking_id:
            return False
        picking_row = self.env['stock.picking'].sudo().search([('id', '=', picking_id)])
        if picking_row and 'OUT' in (picking_row.name).split("/"):
            return True
        return False

    def check_product_is_serial(self, product_id=None):
        if not product_id:
            return
        product_product = self.env['product.product'].search([('id', '=', product_id)])
        # print(product_product.product_tmpl_id)
        # product_temp = self.env['product.template'].search([('id', '=', product_product.product_tmpl_id.id)])
        if 'serial' == product_product.product_tmpl_id.tracking:
            return True
        return False

    def get_quant_id_if_not_reserved(self, lot_id=None, lot_name=None):
        ...
        quant_id = self.env['stock.quant'].sudo().search([('lot_id', '=', lot_id), ('reserved_quantity', '!=', 1)])

        if quant_id:
            return quant_id
        return False
    def get_quant_id_if_not_reserved_delivery(self, lot_id=None,location_id=None, lot_name=None):
        print('check_lot_id_reserve_or_not')
        if lot_name and not lot_id:
            lot_id = self.env['stock.production.lot'].sudo().search('name', '=', lot_name)
        quant_id = self.env['stock.quant'].sudo().search([('lot_id', '=', lot_id), ('reserved_quantity', '!=', 1),('location_id','=',location_id)])
    ...
    def write(self, vals):
        print('product move val (write) :', vals)
        if 'move_line_ids' in vals:
            move_lines = vals.get('move_line_ids')
            if isinstance(move_lines, list):
                for single_move_line in move_lines:
                    dict_move_line = single_move_line[2]
                    if isinstance(dict_move_line, dict):
                        if self.check_if_move_come_from_transfer(
                                dict_move_line.get('picking_id', None)) and self.check_product_is_serial(
                            dict_move_line.get('product_id', None)):
                            dict_move_line['product_uom_qty'] = 1
                            get_stock_quant = self.get_quant_id_if_not_reserved(dict_move_line.get('lot_id', None))
                            if get_stock_quant:
                                get_stock_quant.write({'reserved_quantity': 1})
                        if self.check_if_move_come_from_deliver(
                                dict_move_line.get('picking_id', None)) and self.check_product_is_serial(
                            dict_move_line.get('product_id', None)):
                            dict_move_line['product_uom_qty'] = 1
                            get_stock_quant = self.get_quant_id_if_not_reserved_delivery(dict_move_line.get('lot_id', None),dict_move_line.get('location_id', None))
                            if get_stock_quant:
                                get_stock_quant.write({'reserved_quantity': 1})

        print('product move val after (write) :', vals)
...
#
#
# class InharitStockMove(models.Model):
#     _inherit = 'stock.move'
#
#     def _action_assign(self):
#         """ Reserve stock moves by creating their stock move lines. A stock move is
...
#         partially available.
#         """
#         is_product_is_zero_or_less = False
#         assigned_moves = self.env['stock.move']
#         partially_available_moves = self.env['stock.move']
#         # Read the `reserved_availability` field of the moves out of the loop to prevent unwanted
#         # cache invalidation when actually reserving the move.
...
#                                                                                          package_id=forced_package_id)
#                     if available_quantity <= 0:
#                         print('come stock move qnty is zero ')
#                         is_product_is_zero_or_less = True
#                         continue
...
#             if move.product_id.tracking == 'serial':
#                 move.next_serial_count = move.product_uom_qty
#         print('product move  : ', move_line_vals_list)
#
#         self.env['stock.move.line'].create(move_line_vals_list)
#         partially_available_moves.write({'state': 'partially_available'})
#         assigned_moves.write({'state': 'assigned'})
#         self.mapped('picking_id')._check_entire_pack()
#         return is_product_is_zero_or_less
#
#     def create(self, vals_list):
#         print('product move val (create) :', vals_list)
#         return super(InharitStockMove, self).create(vals_list)
#
...
#
# #     def action_show_details_fpo(self):
# #         """ Returns an action that will open a form view (in a popup) allowing to work on all the
# #         move lines of a particular move. This form view is used when "show operations" is not
# #         checked on the picking type.
# #         """
...
# #         # fetch both.
# #         if self.picking_id.picking_type_id.show_reserved:
# #             view = self.env.ref('usl_recived_forigin_purchased.view_stock_move_operations_new')
# #         else:
# #             view = self.env.ref('stock.view_stock_move_nosuggest_operations')
# #
# #         picking_type_id = self.picking_type_id or self.picking_id.picking_type_id
# #         return {
# #             'name': _('Detailed Operations new'),
# #             'type': 'ir.actions.act_window',
# #             'view_mode': 'form',
# #             'res_model': 'stock.move',
# #             'views': [(view.id, 'form')],
...
# #     #
# #     #     picking_dict=dict()
# #     #     for value in self.picking_id:
# #     #         # picking_dict[]
# #     #         print(value.fields_get())
# #     #         for line in value.fields_get():
# #     #             if line != 'id':
# #     #                 print(line)
# #     #                 print(type(value[line]))
# #     #                 if isinstance(value[line], (int , str, datetime)):
# #     #                     picking_dict[line] = value[line]
# #     #
# #     #                 else:
# #     #                     if line in ['move_lines', 'move_ids_without_package']:
# #     #                         picking_dict[line] = value[line]
# #     #                     else :
# #     #                         picking_dict[line]=value[line].id
# #     #
# #     #     self.env['stock.picking'].create(picking_dict)
# #     #     print('***********************')
# #     #     print('***********************')
# #
# #     def fpo_confirm(self):
# #         for rec in self:
# #             get_stock_move=self.env['stock.move'].search([('product_id','=',rec.product_id.id),('location_id','=',4)])
# #             get_existing_data=self.env['stock.move.line'].search([('move_id','in',get_stock_move.ids),('lot_id','=',None),('qty_done','=',1)],limit=rec.product_uom_qty)
# #             # for line in get_existing_data:
# #             # for line in get_existing_data:
...
# #             # need that if the picking type disallows the creation of new lots.
# #             return
# #         if self.picking_id.is_nonsalealewarehouse_transfar:
# #             return
# #
# #         breaking_char = '\n'
# #         if self.picking_type_id.show_reserved:
