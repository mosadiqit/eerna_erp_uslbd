...
class CommercialPurchaseOrder(models.Model):
    _name = "commercial.purchase.order"
    _inherit = ['mail.thread', 'mail.activity.mixin', 'portal.mixin']
    _description = "Commercial Purchase Order"
    ...
    order_line = fields.One2many('commercial.purchase.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True)
    ...
    def _compute_access_url(self):
        super(CommercialPurchaseOrder, self)._compute_access_url()
        for order in self:
    ...
    def create(self, vals):
        ...
        return super(CommercialPurchaseOrder, self).create(vals)

    def write(self, vals):
        res = super(CommercialPurchaseOrder, self).write(vals)
    ...
    def unlink(self):
        ...
        return super(CommercialPurchaseOrder, self).unlink()

    def copy(self, default=None):
        ctx = dict(self.env.context)
        ctx.pop('default_product_id', None)
        self = self.with_context(ctx)
        new_po = super(CommercialPurchaseOrder, self).copy(default=default)
    ...
    def _track_subtype(self, init_values):
        ...
        return super(CommercialPurchaseOrder, self)._track_subtype(init_values)
    ...
    def message_post(self, **kwargs):
        ...
        return super(CommercialPurchaseOrder, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)
...
class CommercialPurchaseOrderLine(models.Model):
    _name = 'commercial.purchase.order.line'
    _description = 'Commercial Purchase Order Line'
    ...
    order_id = fields.Many2one('commercial.purchase.order', string='Order Reference', index=True, required=True, ondelete='cascade')
    ...
    def create(self, values):
        ...
        line = super(CommercialPurchaseOrderLine, self).create(values)
    ...
    def write(self, values):
        ...
        return super(CommercialPurchaseOrderLine, self).write(values)

    def unlink(self):
        for line in self:
            if line.order_id.state in ['purchase', 'done']:
                raise UserError(_('Cannot delete a purchase order line which is in state \'%s\'.') % (line.state,))
        return super(CommercialPurchaseOrderLine, self).unlink()
