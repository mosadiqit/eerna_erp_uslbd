...
class HelpdeskTicket(models.Model):
    ...
    oem_case_id = fields.Char(string='OEM Case ID')
    product_serial_no = fields.Char(string='Product Serial')
    eng_learner_id = fields.Char(string='Eng. Learner ID')
    contract_type = fields.Selection(selection=[
        ('standard', _('Standard')),
        ('care_pack', _('Care Pack')),
        ('others', _('Others')),
    ], string='Contract Type', default='standard')

    call_log_date_time = fields.Datetime(string='Call Log Date & Time')
    call_type = fields.Selection(selection=[
        ('HW', _('Hardware')),
        ('SW', _('Software')),
        ('Others', _('Others')),
    ], string='Call Type')
    warranty_status = fields.Selection(selection=[
        ('Active', _('Active')),
        ('Expire', _('Expire')),
        ('others', _('Others')),
    ...
    service_ids = fields.One2many('helpdesk.service', 'helpdesk_ticket_id', string='Service Details')
    defective_spare_parts = fields.One2many('helpdesk.defective.spare.part', 'helpdesk_ticket_id', string='Defective Spare Part')
    replacement_spare_parts = fields.One2many('helpdesk.replacement.spare.part', 'helpdesk_ticket_id', string='Replacement Spare Part')
    ...
    customer_rating = fields.Selection(selection=[
        ...
        ('9', _('9')),
        ('10', _('10')),
    ], string='Customer Rating', default='10')
    customer_remarks = fields.Text(string='Customer Remarks')

    comments_by_helpdesk = fields.Text(string='Comments by Service desk/Helpdesk After Verification')
    comments_by_manager = fields.Text(string="Manager/TL's Action & Remarks")

    active = fields.Boolean('Active', default=True)

    def send_user_mail(self):
        self.env.ref('de_helpdesk.assignment_email_template').send_mail(self.id)
