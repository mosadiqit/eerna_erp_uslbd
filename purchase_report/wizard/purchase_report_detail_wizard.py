...
class PurchaseReportDetail(models.TransientModel):
    _name = "purchase.report.detail"
    _description = "Purchase Detail Report .xlsx"
    ...
    product_ids = fields.Many2many('product.product', 'purchase_report_detail_product_rel', 'purchase_report_detail_id',
                                   'product_id', 'Products')
    categ_ids = fields.Many2many('product.category', 'purchase_report_detail_categ_rel', 'purchase_report_detail_id',
                                 'categ_id', 'Categories')
    group_ids = fields.Many2many('product.group', 'purchase_report_detail_group_rel', 'purchase_report_detail_id',
                                 'group_id', 'Group')
    brand_ids = fields.Many2many('product.brand', 'purchase_report_detail_brand_rel', 'purchase_report_detail_id',
                                 'brand_id', 'Brand')
    model_ids = fields.Many2many('product.model', 'purchase_report_detail_model_rel', 'purchase_report_detail_id',
                                 'model_id', 'Model')
    location_ids = fields.Many2many('res.branch', 'purchase_report_branch_rel', 'purchase_report_detail_id',
                                    'branch_id', 'Branch')
    ...
    def print_excel_report(self):
        ...
        columns = [
            ('No', 5, 'no', 'no'),
            ('Branch', 30, 'char', 'char'),
            ('Purchase Person', 20, 'char', 'char'),
            ('Purchase Order', 20, 'char', 'char'),
            ...
            ('Purchase Date', 20, 'datetime', 'char'),
        ]
        ...
        query = """
            ...
            from purchase_order_line as ol
            left join purchase_order as so on ol.order_id = so.id
            ...
            where so.invoice_status = 'invoiced' and ac.state='posted' and so.create_date::date between '{}' and '{}' and
                {} and {} and {} and {} and {} and {}
            ORDER BY 
                sodate
