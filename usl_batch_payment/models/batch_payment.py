    ...
    def _onchange_collection_reference(self):
        if self.collection_reference == 'previous_invoice':
            query = """                        
                                    ...
                                    and amr.invoice_id is null order by id asc""".format(self.partner_id.id)

            self._cr.execute(query=query)
            exist_challan = self._cr.fetchall()
            if not exist_challan:
                self.env.user.notify_warning(message='You have not any Previous Challan. Your Paid amount will be in Advance Payment')
