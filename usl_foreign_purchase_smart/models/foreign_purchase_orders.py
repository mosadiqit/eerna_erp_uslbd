...
class ForeignPurchaseOrder(models.Model):
    ...
    invoice_ids = fields.Many2many('account.move', compute="_compute_invoice", string='Bills',ondelete="cascade", copy=False, store=True)
    ...
    def action_shipment(self):
        ...
        for rec in self:
            for line in rec.line_ids:
                ...
                if line.price_unit == line.bank_payment + line.local_payment:
                    line.remaining_bank_payment=line.bank_payment*line.original_uom_qty
        self.state = 'shipment'
    ...
    def get_product_count(self):
        for rec in self:
            rec.product_count = len(rec.mapped("line_ids"))
    ...
    def action_pre_shipment(self):
        ...
        for rec in self:
            for line in rec.line_ids:
                ...
                line.remaining_bank_payment = line.price_unit*line.original_uom_qty
            ...
            if order.purchase_count > 0:
                for po in order._get_purchase_orders():
                    if po.state not in ("cancel"):
                        raise UserError(
                            _(
                                "You can not delete a  order with opened "
                                "purchase orders! "
...
class ForeignPurchaseOrderLine(models.Model):
    ...
    lc_number=fields.Many2many('usl.lc.management','foreign_purchase_lc_rel',string="L/C No", ondelete='')
            ...
            if len(rec.ids) > 0:
                ...
                if flag==0:
                    ...
                    if len(rec.lc_number)>0:
                        get_exists_all_p_w_lc_h=self.env['purchase.wise.lc.history'].search_count([('po_line_id','=',rec.ids[0])])
                        if len(rec.lc_number)>get_exists_all_p_w_lc_h:
                            for lc_id in rec.lc_number.ids:
                                get_exists_in_p_w_lc_h=self.env['purchase.wise.lc.history'].search([('po_line_id','=',rec.ids[0]),('lc_id','=',lc_id)])
                                ...
                                if not get_exists_in_p_w_lc_h:
                                    ...
                                    if remaining_bank_payment!=0:
                                        if lc_remaining_amount<remaining_bank_payment:
                                            remaining_bank_payment_after_lc = remaining_bank_payment - lc_remaining_amount

                                            # update foreign_purchase_order_line
                                            query="""update foreign_purchase_order_line set remaining_bank_payment={} where id={}""".format(remaining_bank_payment_after_lc,rec.ids[0])
                                            self._cr.execute(query=query)
                                            self._cr.commit()

                                            # Update usl_lc_management
                                            query = "update usl_lc_management set remaining_amount={},reserved_amount={} where id={}".format(
                                                lc_details.remaining_amount-lc_remaining_amount,
                                                lc_remaining_amount, lc_id)
                                            self._cr.execute(query=query)
                                            self._cr.commit()
                                            ...
                                            query = "insert into foreign_purchase_lc_rel values({},{})".format(rec.ids[0],lc_id)
                                            self._cr.execute(query=query)
                                            self._cr.commit()
                        ...
                        if len(rec.lc_number)<get_exists_all_p_w_lc_h:
                            get_deleted_relational_data=self.env['purchase.wise.lc.history'].search([('lc_id','not in',tuple(rec.lc_number.ids)),('po_line_id','=',rec.ids[0])])
                            lc_details = self.env['usl.lc.management'].search([('id', '=', get_deleted_relational_data.lc_id)])
                            #update purchase_order_line
                            po_line_qty=rec.original_uom_qty
                            remaining_bank_payment=rec.remaining_bank_payment+get_deleted_relational_data.taking_amount_from_lc
                            query = """update foreign_purchase_order_line set remaining_bank_payment={} where id={}""".format(
                            ...
                            query = "update usl_lc_management set remaining_amount={},reserved_amount={} where id={}".format(
                                ...
                                lc_details.reserved_amount - get_deleted_relational_data.taking_amount_from_lc, get_deleted_relational_data.lc_id)
                            self._cr.execute(query=query)
                            self._cr.commit()

                            # delete purchase_wise_lc_history
                            query="""delete from purchase_wise_lc_history where po_line_id={} and lc_id={}""".format(rec.ids[0],get_deleted_relational_data.lc_id)
                            self._cr.execute(query=query)
                            self._cr.commit()
                    ...
                    if len(rec.lc_number)==0:
                        ...
                        get_tracking_data=self.env['purchase.wise.lc.history'].search([('po_line_id','=',rec.ids[0])])
                        if get_tracking_data:
                            for trck_data in get_tracking_data:
                                ...
                                get_lc_data=self.env['usl.lc.management'].search([('id','=',trck_data.lc_id)])
                                ...
                                query = "update usl_lc_management set remaining_amount={},reserved_amount={} where id={}".format(
                                    get_lc_data.remaining_amount + taking_amount_from_lc,
                                    get_lc_data.reserved_amount - taking_amount_from_lc, trck_data.lc_id)
                                self._cr.execute(query=query)
                                self._cr.commit()
                                remaining_bank_payment = taking_amount_from_lc
                                query = "update foreign_purchase_order_line set remaining_bank_payment={} where id={}".format(
    ...
    # @api.onchange('lc_number')
    # def check_amount_satisfied_with_bank_payment1(self):
    #
    #     for rec in self:
    #         print(rec.ids)
    #         if len(rec.ids) > 0:
    ...
    #                     # self._cr.execute(query=query)
    #                     # get_tracking_data = self._cr.fetchall()
    #                     get_tracking_data=self.env['purchase.wise.lc.history'].search([('po_line_id','=',rec.ids[0]),('lc_id','=',rec.lc_number.ids[0])])
    #                     get_tracking_data_all = self.env['purchase.wise.lc.history'].search(
    #                         [('po_line_id', '=', rec.ids[0])])
    #                     if get_tracking_data:
    #
    #                         total_taking_amount_from_lc=0
    #                         for value in get_tracking_data_all:
    #                             for value_new in get_tracking_data:
    #                                 if value.lc_id!=value_new.lc_id:
    #                                     total_taking_amount_from_lc+=value.taking_amount_from_lc
    #                         if total_taking_amount_from_lc<(rec.bank_payment*rec.original_uom_qty):
    #                         # ********************************newly added*****************************
    ...
    #                                             'po_line_id':rec.ids[0],
    #                                             'lc_id':lc.ids[0],
    #                                             'taking_amount_from_lc':float(reserved_amount)
    #                                         }]
    #                                         purchase_wise_lc_history.sudo().create(val_list)
    ...
    #                                         }
    #                         else:
    #                             raise ValidationError(_("Product satisfied with lc amount, No need to add another lc"))
    #                         # error_msg = "Product satisfied with lc amount, No need to add another lc"
    #                         # # raise ValidationError(error_msg)
    #                         # return {
    #                         #     'warning': {
    #                         #         'title': 'Warning!',
