...
class ValidateLotNumber(models.Model):
    ...
    def validate_lots(self, lots):
        ...
        for lot in lots:
            self.env.cr.execute("select * from stock_production_lot where UPPER(name) =UPPER(%s)",([lot]))
            quant_st = self.env.cr.dictfetchall()
            if quant_st.__len__() == 0:
                 return ['no_stock', lot]

            for lot_id in quant_st:
                # lot_id = LotObj.search([('name', '=', lot)], limit=1)
                lot_id = LotObj.search([('id', '=', lot_id['id'])], limit=1)
                try:
