...
class MsReportStock(models.TransientModel):
    ...
    categ_ids = fields.Many2many('product.category', 'ms_report_stock_categ_rel', 'ms_report_stock_id',
                                 'categ_id', 'Categories')
    group_ids = fields.Many2many('product.group', 'ms_report_stock_group_rel', 'ms_report_stock_id',
                                 'group_id', 'Group')
    brand_ids = fields.Many2many('product.brand', 'ms_report_stock_brand_rel', 'ms_report_stock_id',
                                 'brand_id', 'Brand')
    model_ids = fields.Many2many('product.model', 'ms_report_stock_model_rel', 'ms_report_stock_id',
                                 'model_id', 'Model')
    ...
    def _get_user_location(self):
        ...
        if self.user_has_groups('stock.group_stock_manager'):
            m_location_id = self.env['stock.location'].search([('usage','=','internal'),('branch_id','=',self.env.user.branch_ids.ids)])
    ...
    def print_excel_report(self):
        ...
        if model_ids:
            where_model_ids = " pmodel.id in %s" % str(tuple(model_ids)).replace(',)', ')')
        ...
        columns = [
            ...
            ('Product Group', 20, 'char', 'char'),
            ('Product Brand', 20, 'char', 'char'),
            ('Product Model', 20, 'char', 'char'),
        ...
        query = """
            SELECT
                ...
                pgroup.name as prod_group,
                pbrand.name as prod_brand,
                pmodel.name as prod_model,
            ...
            LEFT JOIN
                product_group pgroup on pgroup.id=prod_tmpl.group_id
            LEFT JOIN
                product_brand pbrand on pbrand.id=prod_tmpl.brand_id
            LEFT JOIN
                product_model pmodel on pmodel.id=prod_tmpl.product_model_id
            WHERE
                %s and %s and %s and %s and %s
            GROUP BY
                prod_group,prod_brand,prod_model,product, prod_categ, location, date_in
            ORDER BY
                prod_group
        """

        self._cr.execute(query % (hours, hours, where_product_ids, where_location_ids,where_group_ids,where_brand_ids,where_model_ids))
        ...
        #             pmodel.name as prod_model,
        #             loc.complete_name as location,
        #             quant.write_date + interval '%s' as date_in,
        #             date_part('days', now() - (quant.write_date + interval '%s')) as aging,
        #             sum(quant.quantity) as total_product,
        #             sum(quant.quantity-quant.reserved_quantity) as stock,
        #             sum(quant.reserved_quantity) as reserved
        #         FROM
        ...
        #     hours, hours, where_product_ids, where_location_ids, where_group_ids, where_brand_ids, where_model_ids))
        # result = self._cr.fetchall()
