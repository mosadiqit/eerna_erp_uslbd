...
class ForeignAccountMove(models.Model):
    ...
    def action_view_relavent_costing(self):
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'views': [(self.env.ref('usl_foreign_purchase.ci_invoice_tree').id, 'tree'),(self.env.ref('usl_foreign_purchase.ic_invoice_form').id, 'form'),(False, 'kanban')],
    ...
    def cal_landed_cost(self):
        ...
        action = self.env.ref('usl_foreign_purchase.ic_invoice_action').read()[0]
        ...
        views = [(self.env.ref('usl_foreign_purchase.ic_invoice_form').id, 'form'),(False, 'kanban')]
    ...
    #     # OVERRIDE
    #     moves = super(ForeignAccountMove, self).create(vals_list)
    #     for move in moves:
    #         if move.reversed_entry_id:
    #             continue
    #         purchase = move.line_ids.mapped('foreign_purchase_line_id.order_id')
    #         if not purchase:
    #             continue
    ...
    def write(self, vals):
        ...
        #                     pur_pro.invoiced_uom_qty+=inv_pro.quantity
                ...
                # picking_name_seq = picking_type[:1].sequence_id.next_by_id()
                # for rec in self.invoice_line_ids:
                #     self.env['foreign.stock.quant.without.serial'].create({'name':picking_name_seq,'invoice_id':self.id,'foreign_purchase_id':self.foreign_purchase_id.id,
                #                                                            'product_id':rec.product_id.id,'quantity':rec.quantity,'price':rec.price_unit})
    ...
    # def approve_foreign_invoice_order(self):
    ...
    def approve_invoice_order(self):
        ...
        if active_model == 'foreign.purchase.order':
                ...
                for move in move_id:
                    if move.product_id.id==account_line.product_id.id:
                        if account_line.product_id.tracking=='serial':
                                ...
                                for stock_line in move_id.move_line_ids:
                                     ...
                                     if k<=i:
                                        if stock_line.product_id.id==account_line.product_id.id and stock_line.qty_done==0 and stock_line.move_id.id==move.id:
                                            ...
                                            self._cr.commit()
                                            # max_id=0
                                            # query="select max(id) from stock_quant"
                                            # self._cr.execute(query=query)
                            ...
                            # else:
                            #     max_id = id[0] + 1
                            # query_stock_quant_vendorLine = """insert into stock_quant values({},{},{},{},null,null,null,{},{},'{}',{},'{}',{},'{}',null,null,null,null)""".format(
                            #     max_id, account_line.product_id.id, self.env.user.company_id.id, vendor_id, neg_qty_done, 0,
                            #     datetime.today(), self.env.user.id, datetime.today(), self.env.user.id,
                            #     datetime.today(), None, None, None, None)
                            # query_stock_quant_other = """insert into stock_quant values({},{},{},{},null,null,null,{},{},'{}',{},'{}',{},'{}',null,null,null,null)""".format(
                            #     max_id + 1, account_line.product_id.id, self.env.user.company_id.id, user_default_warehouse,
                            #     current_receive_qty, 0, datetime.today(), self.env.user.id,
                            #     datetime.today(), self.env.user.id, datetime.today())
                            # print(query_stock_quant_vendorLine)
                            # self._cr.execute(query=query_stock_quant_vendorLine)
                            # self._cr.execute(query=query_stock_quant_other)
            ...
            # str(tuple(move_id.ids)).replace(',)', ')'))
            # self._cr.execute(query=query_move)
            # query_move_line = """update stock_move_line set qty_done = {},product_qty=0,product_uom_qty=0, state='done', location_dest_id = '72' where move_id in {}""".format(
            # str(tuple(move_id.ids)).replace(',)', ')'))
            # self._cr.execute(query=query_move_line)
            if self.state == 'posted':
                get_account_move_line = self.invoice_line_ids
