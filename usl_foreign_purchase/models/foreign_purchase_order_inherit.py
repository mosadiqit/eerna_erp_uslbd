...
class ForeignPurchaseOrderInherit(models.Model):
    ...
    def write(self, vals):
        if vals.get('line_ids') and self.state == 'purchase':
            for order in self:
                pre_line_ids_qty = {line_ids: line_ids.product_qty for line_ids in order.mapped('line_ids')}
        res = super(ForeignPurchaseOrderInherit, self).write(vals)
        if vals.get('line_ids') and self.state == 'purchase':
            for order in self:
                to_log = {}
                for line_ids in order.line_ids:
                    if pre_line_ids_qty.get(line_ids, False) and float_compare(pre_line_ids_qty[line_ids], line_ids.product_qty, precision_rounding=line_ids.product_uom.rounding) > 0:
                        to_log[line_ids] = (line_ids.product_qty, pre_line_ids_qty[line_ids])
                if to_log:
    ...
    def button_approve_svl(self, force=False):
        # result = super(ForeignPurchaseOrderInherit, self).button_approve(force=force)
        self._create_picking()
        # self._create_stock_valuation_layer()
    ...
    def _create_stock_valuation_layer(self):
        ...
        for line in self.line_ids:
            query = "select max(id) from stock_valuation_layer"
            ...
            product_qty=line.original_uom_qty
            unit_cost=line.price_unit*self.env.ref('base.main_company').currency_id.rate
            value=line.price_subtotal*self.env.ref('base.main_company').currency_id.rate
            remaining_qty=line.original_uom_qty
            remaining_value=line.price_subtotal*self.env.ref('base.main_company').currency_id.rate

            query="select sp.name,sp.id,sm.id from stock_picking sp" \
                  " left join stock_move sm on sm.picking_id=sp.id where sp.origin='{}'".format(self.name)
            self._cr.execute(query=query)
            picking=self._cr.fetchone()
            description =picking[0]+' - '+line.name
            stock_valuation_layer_id=None
            stock_move_id=picking[2]
            account_move_id=None
            create_uid=self.env.user.id
            create_date=datetime.today()
            write_uid=self.env.user.id
            write_date=datetime.today()
            stock_landed_cost=None

            query="insert into stock_valuation_layer " \
                  "values({},{},{},{},{},{},{},{},'{}',NULL,{},NULL,{},'{}',{},'{}',NULL)".format(max_id,company_id,product_id,product_qty,unit_cost,value,remaining_qty,remaining_value,
                                                                                      description,stock_move_id,create_uid,create_date,write_uid,
                                                                                      write_date)
            print(query)
            self._cr.execute(query=query)
            self._cr.commit()
    ...
    def button_cancel(self):
        for order in self:
            for move in order.line_ids.mapped('move_ids'):
                if move.state == 'done':
            ...
            if order.state in ('draft', 'sent', 'to approve', 'purchase'):
                for line_ids in order.line_ids:
                    line_ids.move_ids._action_cancel()
                    if line_ids.move_dest_ids:
                        move_dest_ids = line_ids.move_dest_ids
                        if line_ids.propagate_cancel:
            ...
            order.line_ids.write({'move_dest_ids':[(5,0,0)]})

        return super(ForeignPurchaseOrderInherit, self).button_cancel()
    ...
    def _log_decrease_ordered_quantity(self, purchase_line_idss_quantities):
        ...
        def _render_note_exception_quantity_po(order_exceptions):
            line_ids_ids = self.env['purchase.order.line'].browse([line_ids.id for order in order_exceptions.values() for line_ids in order[0]])
            purchase_order_ids = line_ids_ids.mapped('order_id')
        ...
        documents = self.env['stock.picking']._log_activity_get_documents(purchase_line_idss_quantities, 'move_ids', 'DOWN', _keys_in_sorted, _keys_in_groupby)
        filtered_documents = {}
    ...
    def _get_destination_location(self):
        ...
        query="""select sl.location_id from res_branch_res_users_rel rb
                inner join stock_location sl on sl.branch_id=rb.res_branch_id
                 where rb.res_users_id={} and sl.company_id={}""".format(self.env.user.id,self.env.user.company_id.id)
        self._cr.execute(query=query)
        destination_id = self._cr.fetchone()
...
class ForeignPurchaseOrderLine(models.Model):
    _inherit = 'foreign.purchase.order.line'
    ...
    def _compute_qty_received(self):
        super(ForeignPurchaseOrderLine, self)._compute_qty_received()
        for line in self:
    ...
    def create(self, values):
        line = super(ForeignPurchaseOrderLine, self).create(values)
    ...
    def write(self, values):
        ...
        result = super(ForeignPurchaseOrderLine, self).write(values)
    ...
    # def _get_stock_move_price_unit_foreign(self):
    #     self.ensure_one()
    #     line = self[0]
    #     order = line.order_id
    ...
    #             price_unit, order.company_id.currency_id, self.company_id, self.create_date or fields.Date.today(),
    #             round=False)
    #     return price_unit
    ...
    def _get_stock_move_price_unit(self):
        ...
        order = line.order_id
        # price_unit = line.price_unit
        # local_rate=self.env['res.currency'].search([('id','=',self.context)]).local_currency
        price_unit = (line.bank_payment*self.env.ref('base.main_company').currency_id.rate) +(line.local_payment*self.env.ref('base.main_company').currency_id.local_currency)
        # if line.taxes_id:
        #     price_unit = line.taxes_id.with_context(round=False).compute_all(
        ...
        # if order.currency_id != order.company_id.currency_id:
        #     price_unit = order.currency_id._convert(
        #         price_unit, order.company_id.currency_id, self.company_id, self.create_date or fields.Date.today(), round=False)
        return price_unit
    ...
    def _prepare_stock_moves(self, picking):
        ...
        diff_quantity = self.original_uom_qty - qty
...
class StockMoveInherit(models.Model):
    ...
    foreign_purchase_line_id = fields.Many2one('foreign.purchase.order.line',
