odoo.define('usl_office_food_cataring_service_management.kiosk_confirm', function (require) {
"use strict";
...
var UslKioskConfirm = AbstractAction.extend({
    events: {
        ...
        'click .o_hr_attendance_pin_pad_button_ok': _.debounce(function() {
            ...
            this._rpc({
                    model: 'hr.employee',
                    method: 'meal_eat_manual',
                    args: [[this.employee_id], this.$('.o_hr_attendance_PINbox').val()],
                })
                .then(function(result) {
                    if (result.action) {
                        self.do_action(result.action);
    ...
    init: function (parent, action) {
        ...
        this.next_action = 'usl_office_food_cataring_service_management.test_kiosk_mode';
...
core.action_registry.add('usl_kiosk_confirm', UslKioskConfirm);

return UslKioskConfirm;
