...
class ReportGeneralLedger(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account,coa_ids,company_id):
        print(company_id)
        """
        :param:
        ...
        if init_balance:
            ...
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, 0.0 AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
                            ...
                            JOIN account_journal j ON (l.journal_id=j.id)\
                            WHERE  m.company_id= %s and l.account_id = %s""" + filters + '  GROUP BY l.account_id')
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            ...
            JOIN account_journal j ON (l.journal_id=j.id)\
            JOIN account_account acc ON (l.account_id = acc.id) \
            WHERE m.company_id=%s and l.account_id = %s ''' + filters + ''' GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.ref, l.name, m.name, c.symbol, p.name ORDER BY ''' + sql_sort)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        accounts_res = self.with_context(data['form'].get('used_context',{}))._get_account_move_entry(accounts, init_balance, sortby, display_account,coa_ids,company_id)
        return {
            'doc_ids': docids,
