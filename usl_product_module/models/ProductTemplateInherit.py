...
class ProductTemplateInherit(models.Model):
    ...
    probational_product = fields.One2many('probational.attribute.setup','product_id','Product')
    ...
    def create(self, vals_list):
        ...
        if 'probational_product' in vals_list[0].keys():
            ...
            for val in probational_products:
                ...
                val_line={
                    'id':max_id,
                    'product_id':templates.id,
                    'probational_product_id':val[2]['probational_product_id'],
                    'percentage':val[2]['percentage'],
                    'create_uid':self.env.user.id,
                    'create_date':str(datetime.today()),
                    'write_uid':self.env.user.id,
                    'write_date':str(datetime.today())
                }
                provision_line.append(val_line)
                # query = "insert into probational_attribute_setup " \
                #         "values ({},{},{},{},{},'{}',{},'{}')".format(max_id, templates.id, val[2]['probational_product_id'],
                #                                                       val[2]['percentage'], self.env.user.id,
                #                                                       str(datetime.today()), self.env.user.id,
                #                                                       str(datetime.today()))
                # print(query)
                # self._cr.execute(query=query)
                self.env['probational.attribute.setup'].create(provision_line)
                self._cr.commit()
            ...
            if 'probational_product' in vals.keys():
                for val in vals['probational_product']:
                    ...
                    if val[0]==0:
                        ...
                        existing_id=val[1]
                        get_existing = self.env['probational.attribute.setup'].search([('id', '=', existing_id)])
                        if val[2]:
                            if 'product_probational_tmpl_id' in val[2].keys():

                                get_existing[0].probational_product_id=val[2]['probational_product_id']
                            ...
                            if 'percentage' in val[2].keys():
                                get_existing[0].percentage = val[2]['percentage']
                            ...
                            if val[0] == 2:
                                query="""delete from probational_attribute_setup where id={}""".format(val[1])
                                self._cr.execute(query=query)
                                self._cr.commit()
...
class ProbationalAttributeSetup(models.Model):
    ...
    probational_product_id = fields.Many2one('product.template', domain=[('probational_cost_ok','=',True)])
...
class ProductProductInherit(models.Model):
    ...
    previous_standard_price = fields.Float()
